...
class IfCategoryExaminationResource extends Resource
{
                ...
                Select::make('member_id')
                    ...
                    ->getSearchResultsUsing(function (string $search) {
                        return \App\Models\Member::query()
                            ->with('category') // Eager load biar efisien
                            ->where('member_name', 'like', "%{$search}%")
                            ->orWhereHas('category', fn($q) => $q->where('name', 'like', "%{$search}%")) // ← FIXED bagian ini
                            ->limit(20)
                            ->get()
                            ->mapWithKeys(function ($member) {
                                ...
                                return [
                                    $member->id => "{$member->member_name} - " . ($member->category->name ?? '-') . " - {$ageText}"
                                ];
                            });
                    })
                    ->getOptionLabelUsing(function ($value): ?string {
                        $member = \App\Models\Member::with('category')->find($value); // Pastikan eager load juga di sini
                        if (!$member) return null;

                        $age = \Carbon\Carbon::parse($member->birthdate)->diff(now());
                        $ageText = "{$age->y} tahun {$age->m} bulan";

                        return "{$member->member_name} - " . ($member->category->name ?? '-') . " - {$ageText}";
                    })
                    ->reactive()
                    ->afterStateUpdated(function ($set, $get, $state) {
                        if ($state) {
                            $member = \App\Models\Member::with('category')->find($state);
                            if ($member) {
                                $set('category', $member->category->name ?? null);
