...
class ViewMember extends ViewRecord
{
    ...
    // {
    //     $latestExamination = $this->record->examinations()->latest()->first();
    //     // Eager load checkup relation and order by checkup_date descending
    //     $examinations = $this->record->examinations()
    //         ->with('checkup')
    //         ->join('checkups', 'examinations.checkup_id', '=', 'checkups.id')
    //         ->orderByDesc('checkups.checkup_date')
    //         ->select('examinations.*')
    //         ->get();
    ...
    //         Forms\Components\Section::make('Semua Riwayat Pemeriksaan')
    //             ->schema(
    //                 $examinations->map(
    //                     fn($exam) =>
    //                     Forms\Components\Group::make()
    //                         ->schema([
    //                             Forms\Components\TextInput::make('checkup_date_' . $exam->id)
    //                                 ->label('Tanggal Pemeriksaan')
    //                                 ->default(
    //                                     $exam->checkup?->checkup_date
    //                                         ? Carbon::parse($exam->checkup->checkup_date)->translatedFormat('d F Y')
    //                                         : '-'
    //                                 )
    //                                 ->disabled(),

    //                             Forms\Components\TextInput::make('weight_' . $exam->id)
    //                                 ->label('Berat Badan')
    //                                 ->default($exam->weight ? $exam->weight . ' kg' : '-')
    //                                 ->disabled(),

    //                             Forms\Components\TextInput::make('height_' . $exam->id)
    //                                 ->label('Tinggi Badan')
    //                                 ->default($exam->height ? $exam->height . ' cm' : '-')
    //                                 ->disabled(),

    //                             Forms\Components\TextInput::make('weight_status_' . $exam->id)
    //                                 ->label('Status Gizi')
    //                                 ->default($exam->weight_status ?? '-')
    //                                 ->disabled(),

    //                             Forms\Components\Textarea::make('recommendation_' . $exam->id)
    //                                 ->label('Rekomendasi')
    //                                 ->default($exam->recommendation ?? '-')
    //                                 ->disabled(),
    //                         ])
    //                         ->columns(2)
    //                 )->toArray()
